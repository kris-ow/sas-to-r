...
Traps: the third argument is a stop position, not a length. `substr(x, 5, 3)` is not "three characters from 5"; it is the empty range 5-to-3. Write `start+len-1`, or count the end position. `substring()` and `str_sub()` use end positions too.
